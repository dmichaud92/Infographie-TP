...
//----------------------------------------
ofNode::ofNode()
    : parent(nullptr), localTransformMatrix(1),
...
ofNode::ofNode(const ofNode &node)
    : parent(node.parent), axis(node.axis),
...
ofNode::ofNode(ofNode &&node)
    : parent(node.parent), position(std::move(node.position)),
      orientation(std::move(node.orientation)), scale(std::move(node.scale)),
...
void ofNode::setParent(ofNode &parent, bool bMaintainGlobalTransform) {
  ...
  if (bMaintainGlobalTransform) {
    ...
    auto postParentOrientation =
        orientation.get() * glm::inverse(parent.getGlobalOrientation());
...
//----------------------------------------
ofNode *ofNode::getParent() const { return parent; }
...
void ofNode::setGlobalPosition(const glm::vec3 &p) {
  if (parent == nullptr) {
    ...
    auto newP =
        glm::inverse(parent->getGlobalTransformMatrix()) * glm::vec4(p, 1.0);
    setPosition(glm::vec3(newP) / newP.w);
  }
}

//----------------------------------------
glm::vec3 ofNode::getPosition() const { return position; }

//----------------------------------------
float ofNode::getX() const { return position->x; }

//----------------------------------------
float ofNode::getY() const { return position->y; }

//----------------------------------------
float ofNode::getZ() const { return position->z; }
...
//----------------------------------------
glm::quat ofNode::getOrientationQuat() const { return orientation; }
...
//----------------------------------------
void ofNode::setScale(float s) { setScale(s, s, s); }

//----------------------------------------
void ofNode::setScale(float sx, float sy, float sz) { setScale({sx, sy, sz}); }
...
//----------------------------------------
glm::vec3 ofNode::getScale() const { return scale; }

//----------------------------------------
void ofNode::move(float x, float y, float z) { move({x, y, z}); }
...
//----------------------------------------
void ofNode::truck(float amount) { move(getXAxis() * amount); }

//----------------------------------------
void ofNode::boom(float amount) { move(getYAxis() * amount); }

//----------------------------------------
void ofNode::dolly(float amount) { move(getZAxis() * amount); }

//----------------------------------------
void ofNode::tilt(float degrees) { rotateDeg(degrees, getXAxis()); }

//----------------------------------------
void ofNode::pan(float degrees) { rotateDeg(degrees, getYAxis()); }

//----------------------------------------
void ofNode::roll(float degrees) { rotateDeg(degrees, getZAxis()); }
...
//----------------------------------------
void ofNode::tiltRad(float radians) { rotateRad(radians, getXAxis()); }

//----------------------------------------
void ofNode::panRad(float radians) { rotateRad(radians, getYAxis()); }
...
void ofNode::rotateAround(const glm::quat &q, const glm::vec3 &point) {
  //	ofLogVerbose("ofNode") << "rotateAround(const glm::quat& q, const
  //ofVec3f& point) not implemented yet"; 	ofMatrix4x4 m =
  //getLocalTransformMatrix(); 	m.setTranslation(point); 	m.rotate(q);
...
//----------------------------------------
void ofNode::rotateAround(float degrees, const glm::vec3 &axis,
                          const glm::vec3 &point) {
  rotateAroundDeg(degrees, axis, point);
}

//----------------------------------------
void ofNode::rotateAroundDeg(float degrees, const glm::vec3 &axis,
                             const glm::vec3 &point) {
  rotateAround(glm::angleAxis(ofDegToRad(degrees), axis), point);
}

//----------------------------------------
void ofNode::rotateAroundRad(float radians, const glm::vec3 &axis,
                             const glm::vec3 &point) {
...
void ofNode::lookAt(const glm::vec3 &lookAtPosition, glm::vec3 upVector) {
  if (parent) {
    auto upVector4 = glm::inverse(parent->getGlobalTransformMatrix()) *
                     glm::vec4(upVector, 1.0);
...
void ofNode::updateAxis() {
  if (scale->x > 0)
    axis[0] = glm::vec3(getLocalTransformMatrix()[0] / scale->x);
  if (scale->y > 0)
    axis[1] = glm::vec3(getLocalTransformMatrix()[1] / scale->y);
  if (scale->z > 0)
    axis[2] = glm::vec3(getLocalTransformMatrix()[2] / scale->z);
}

//----------------------------------------
glm::vec3 ofNode::getXAxis() const { return axis[0]; }

//----------------------------------------
glm::vec3 ofNode::getYAxis() const { return axis[1]; }

//----------------------------------------
glm::vec3 ofNode::getZAxis() const { return axis[2]; }

//----------------------------------------
glm::vec3 ofNode::getSideDir() const { return getXAxis(); }

//----------------------------------------
glm::vec3 ofNode::getLookAtDir() const { return -getZAxis(); }

//----------------------------------------
glm::vec3 ofNode::getUpDir() const { return getYAxis(); }

//----------------------------------------
float ofNode::getPitch() const { return getPitchDeg(); }

//----------------------------------------
float ofNode::getHeading() const { return getHeadingDeg(); }

//----------------------------------------
float ofNode::getRoll() const { return getRollDeg(); }

//----------------------------------------
float ofNode::getPitchDeg() const { return getOrientationEulerDeg().x; }

//----------------------------------------
float ofNode::getHeadingDeg() const { return getOrientationEulerDeg().y; }

//----------------------------------------
float ofNode::getRollDeg() const { return getOrientationEulerDeg().z; }

//----------------------------------------
float ofNode::getPitchRad() const { return getOrientationEulerRad().x; }

//----------------------------------------
float ofNode::getHeadingRad() const { return getOrientationEulerRad().y; }

//----------------------------------------
float ofNode::getRollRad() const { return getOrientationEulerRad().z; }
...
glm::quat ofNode::getGlobalOrientation() const {
  if (parent)
    return parent->getGlobalOrientation() * getOrientationQuat();
...
//----------------------------------------
void ofNode::orbit(float longitude, float latitude, float radius,
...
//----------------------------------------
void ofNode::orbitDeg(float longitude, float latitude, float radius,
                      ofNode &centerNode) {
...
void ofNode::orbitDeg(float longitude, float latitude, float radius,
  ...
  glm::vec4 p{0.f, 0.f, 1.f,
              0.f}; // p is a direction, not a position, so .w == 0
...
//----------------------------------------
void ofNode::orbitRad(float longitude, float latitude, float radius,
                      ofNode &centerNode) {
...
//----------------------------------------
void ofNode::draw() const { ofGetCurrentRenderer()->draw(*this); }
...
//----------------------------------------
void ofNode::customDraw() { legacyCustomDrawOverrided = false; }
...
void ofNode::createMatrix() {
  ...
  localTransformMatrix =
      localTransformMatrix * glm::toMat4((const glm::quat &)orientation);
