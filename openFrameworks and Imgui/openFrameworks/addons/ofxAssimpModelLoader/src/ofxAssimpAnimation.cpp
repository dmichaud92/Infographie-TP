...
ofxAssimpAnimation::ofxAssimpAnimation(shared_ptr<const aiScene> scene,
...
aiAnimation *ofxAssimpAnimation::getAnimation() { return animation; }
...
void ofxAssimpAnimation::updateAnimationNodes() {
  for (unsigned int i = 0; i < animation->mNumChannels; i++) {
    ...
    if (channel->mNumRotationKeys > 0) {
      ...
      if (diffTime > 0) {
        float factor = float((progressInSeconds - key.mTime) / diffTime);
        aiQuaternion::Interpolate(presentRotation, key.mValue, nextKey.mValue,
                                  factor);
      } else {
...
void ofxAssimpAnimation::reset() { setPosition(0); }

bool ofxAssimpAnimation::isFrameNew() {
  return (
      bPlay &&
      !bPause); // assume its always a new frame when playing and not paused.
}

bool ofxAssimpAnimation::isPaused() { return bPause; }

bool ofxAssimpAnimation::isPlaying() { return bPlay; }

bool ofxAssimpAnimation::isFinished() {
  return !bPlay && (getPosition() == 1.0);
}

float ofxAssimpAnimation::getPosition() { return progress; }

float ofxAssimpAnimation::getPositionInSeconds() { return progressInSeconds; }

int ofxAssimpAnimation::getPositionInMilliSeconds() {
  return progressInMilliSeconds;
}

float ofxAssimpAnimation::getSpeed() { return speed; }

float ofxAssimpAnimation::getDurationInSeconds() { return durationInSeconds; }

int ofxAssimpAnimation::getDurationInMilliSeconds() {
  return durationInMilliSeconds;
}

void ofxAssimpAnimation::setPaused(bool paused) { bPause = paused; }
...
void ofxAssimpAnimation::setLoopState(ofLoopType state) { loopType = state; }
